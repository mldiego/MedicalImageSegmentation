...
layers = [imageInputLayer([64 64 1])
    convolution2dLayer(3, 3,'Stride',1,'WeightsInitializer','narrow-normal', 'BiasInitializer','narrow-normal');
    batchNormalizationLayer
    reluLayer
    dropoutLayer
    averagePooling2dLayer(2,'Stride',2,'Padding',[0 0 0 1])
    flattenLayer
    fullyConnectedLayer(6, 'WeightsInitializer','narrow-normal', 'BiasInitializer','narrow-normal')
    softmaxLayer
    classificationLayer];
...
for sd = seeds
    ...
    % Save model
    save("models/model_dropout_narrow-normal_"+string(sd)+".mat", 'net', 'accuracy');
