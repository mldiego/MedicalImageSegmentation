...
%% Load network
netMatlab = importONNXNetwork('models/size_128/best_model.onnx');
...
save('results/r128.mat', 'rT', 'res', "errMsgs");


% Create input set
% IS = ImageStar(slice_img, slice_img);

% Compute reachability
% R = net.reach(IS, reachOptions);


% All of them get out of memory errors...

% Looking into how to reduce memory requirements in each of the layers...
% POSLIN, Concatenation...
% Can we reduce the number of constraints/basis vectors?
% - Could we change the code to make use of sparse / tall arrays?
%       Tall arrays or sparse arrays are not supported in Deep Learning toolbox, so thats a nope
%
% - Can we change the ImageStar data structure to reduce the size of the arrays in general?
% Even the smallest model runs out of memory on Layer 27 (concatenation...)