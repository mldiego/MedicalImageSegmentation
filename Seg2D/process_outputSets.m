...
for k = 1:length(models)
    ...
    for i = 1:N
        ...
        if ~isempty(resData.ME)
            ...
            % b) compute IoU score
            iou_label = jaccard(double(slice_target), double(verified_image));
            iou_label = iou_label(~isnan(iou_label));
            riou_label = sum(iou_label)/length(iou_label);
            iou_pred = jaccard(double(yPred), double(verified_image));
            iou_pred = iou_pred(~isnan(iou_pred));
            riou_pred = sum(iou_pred)/length(iou_pred);
