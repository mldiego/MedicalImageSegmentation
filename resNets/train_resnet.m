...
%   labels : [      1          2             3           4       5      6   ]           
%              
% Net work is a simple residual network
...
% Define layers
% layers = resnetLayers(inputSize,numClasses); % 177 layers...
% Start with a smaller network
layers = resnetLayers(inputSize,numClasses,...
                InitialStride=1, ...
                InitialFilterSize=3, ...
                InitialNumFilters=3, ...
                StackDepth=1, ...
...
options = trainingOptions('adam', 'InitialLearnRate', 1e-3, ...
    'MaxEpochs', 10, 'VerboseFrequency', 100, 'Shuffle','every-epoch', ...
    'ValidationData',{Xval,Yval},'ValidationPatience',Inf);
