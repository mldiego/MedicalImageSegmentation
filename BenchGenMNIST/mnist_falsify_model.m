function mnist_falsify_model(modelpath, xRand, targets)
    ...
    for i=1:N
        ...
        t = tic;
        cEx = falsify_single(nn, xRand, targets(i));
        if isnan(cEx{1})
    ...
    % Save results
    save(['results', filesep, 'falsify_', saveName], 'res');

end

function counterEx = falsify_single(net, xRand, yTarget)
    counterEx = {nan};
    s = size(xRand);
    n = length(s);
